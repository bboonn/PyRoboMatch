...
class RoboYard(threading.Thread):
	...
	def run(self):
		while True:
			...
			time.sleep(idel_max/2)
			if self.yard_queue.empty():
				break
		self.byeBye()

	def childbearingPolicy(max_child = 2):
		for r in self.robo_list:
			if "alive" == r.state:
				self.robo_list.append(r.bornChild(max_child))
				if r.dad:
		...
		def getRoboInfo():
			...
			for r in self.robo_list:
				...
				if lastest_gene < r.gene:
					lastest_gene = r.gene
				if self.peak_power < sum(current_power):
					self.peak_power = sum(current_power)
			...
			if sum(alive):
				printInfo("Power per Alive", self.floatToStr(sum(current_power)/sum(alive)))
			printInfo("Civilization Point",self.floatToStr(peak_power/sum(total)))
